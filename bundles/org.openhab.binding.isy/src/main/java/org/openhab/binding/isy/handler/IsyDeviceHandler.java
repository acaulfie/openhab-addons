...
public class IsyDeviceHandler extends AbtractIsyThingHandler {
    ...
    protected Map<Integer, String> mDeviceidToControlChannelMap = new HashMap<Integer, String>();
    ...
    public void handleUpdate(String control, String action, String node) {
        ...
        } else if (this.mDeviceidToControlChannelMap.containsKey(deviceId)
                && ("DOF".equals(control) || "DFOF".equals(control) || "DON".equals(control) || "DFON".equals(control)
                        || "FDUP".equals(control) || "FDDOWN".equals(control))) {
            String channelID = this.mDeviceidToControlChannelMap.get(deviceId);
            if (channelID != null) {
                updateState(channelID, new StringType(control));
            }
        }
    ...
    public void handleCommand(@Nullable ChannelUID channelUID, Command command) {
        ...
        try {
            ...
            logger.debug("DeviceHandler handleCommand caught exception: {}", e.getMessage());
    ...
    protected void addControlChannel(String channel, int deviceId) {
        this.mDeviceidToControlChannelMap.put(deviceId, channel);
    }
